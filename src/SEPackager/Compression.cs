...
internal class Compression
{
    ...
    public Compression(string[] paths, Mode type)
    {
        mode = type;
        Compress(paths);
    }
    ...
    private void WriteArchiveData(BinaryWriter bw)
    {
        byte partPointer = 0x00;
        uint temp = 0;

        FileStream partFS = new($"{_outPath}{partName}{partPointer:D3}.sep", FileMode.Create, FileAccess.Write);
        BinaryWriter partBW = new(partFS);
        {
            ...
            {
                _files = Path.GetRelativePath(inPath, fullFileName![i]);
                ...
                switch (mode)
                {
                    ...
                    case Mode.tex:
                        {
                            if (ImageGrabber.TryGrabImage(fullFileName[i], out var imgData))        // This porbably causes memory leaks
                            {
                                using (imgData)
                                {
    ...
    private void WriteDirsHeader(BinaryWriter bw)
    {
    ...
    {
        dirsName = name + $"_{Enum.GetName(typeof(Mode), mode)}_dirs.sep";
        partName = name + $"_{Enum.GetName(typeof(Mode), mode)}_";
    }
}