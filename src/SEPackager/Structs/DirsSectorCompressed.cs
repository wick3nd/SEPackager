...
{
    internal class DirsSectorCompressed
    {
        ...
        public uint OriginalLength { get; set; } = 0;

        public ReadOnlyMemory<byte> GetArray()
        {
            byte[] buffer = new byte[Path.Length + 14];
            ...
            buffer[Path.Length + 10] = (byte)(OriginalLength >> 24);
            buffer[Path.Length + 11] = (byte)(OriginalLength >> 16);
            buffer[Path.Length + 12] = (byte)(OriginalLength >> 8);
            buffer[Path.Length + 13] = (byte)OriginalLength;
